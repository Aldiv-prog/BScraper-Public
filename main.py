...
def _load_scraping_session(session_file: str) -> Tuple[List[BlogPost], ScrapingSession]:
    """Load posts and session from saved scraping session file."""
    session = ScrapingSession.load_from_file(session_file)
    if not session:
        raise BScrapeException(f"Failed to load session from {session_file}")
    posts = _posts_from_session(session)
...
def _interactive_post_scraping_workflow(
    ...
    Allows user to choose next steps: continue analysis, inspect data, or quit.

    Returns:
        Next phase to execute: 'summarize', 'traits', 'quit', or 'inspect'
    """
    print("\n" + "=" * 60)
    ...
    print(f"Output files available for inspection:")
    print(f"   - {session_file} (scraping session data)")
    print()
...
def _handle_existing_session_workflow(
    ...
    if saved_session.is_complete:
        print("This scraping session appears to be complete.")
        menu_options = [
        ...
        print("This scraping session appears to be incomplete.")
        menu_options = [
...
def main():
    ...
    parser.add_argument('--phase', choices=['all', 'scrape', 'summarize', 'traits'], default='all',
                        help='Pipeline phase to run (interactive workflow available after scraping)')
    ...
    try:
        ...
        if args.phase in ('all', 'scrape') and not posts:
            ...
            if args.phase == 'scrape':
                ...
                if next_phase == 'quit':
                    logger.info("User chose to quit after scraping")
                    return 0
        ...
        if args.archive:
            analysis_metadata = {
                ...
                'essay_word_count': output_config.get('essay_word_count'),
                'traits_confidence_threshold': output_config.get('traits_confidence_threshold'),
